...
WIN_WIDTH =284 * 2# 284 * 2     pygame.display.Info().current_w # BG image size: 284x512 px; tiled twice 窗口宽度
WIN_HEIGHT =512  # 512    pygame.display.Info().current_h#窗口高度
...
#     CLIMB_DURATION = 333.3 #一个攀升周期（单位：毫秒） 原始：333.3
#     ADD_INTERVAL = 3000 #管子障碍物之间的间隔（单位毫秒） 原始：3000 每隔该时间出现一次障碍物
#     ANIMATION_SPEED= 0.18  #小鸟飞行速度
    ...
    global ADD_INTERVAL
    if ANIMATION_SPEED>=0.12:
        ANIMATION_SPEED -=0.02
        PipePair.ADD_INTERVAL +=PipePair.ADD_INTERVAL*(0.02/ANIMATION_SPEED) #修改间隔使管子出现的像素间隔恒定
...
def makegamehard():
    ...
    global CLIMB_DURATION
    global ADD_INTERVAL
    if ANIMATION_SPEED<=0.24:
        ANIMATION_SPEED +=0.02
        PipePair.ADD_INTERVAL -=PipePair.ADD_INTERVAL*(0.02/ANIMATION_SPEED)
...
def rungame():
    ...
    #*******************************************
    N= 15 #游戏总计关卡数量 即trial数量为N+1，但最后一关有时会出问题，所以不计入总数
    passscore= 7 #每关成功所需通过障碍物的数量7
...
#             pygame.mixer.music.play(-1, 0.0)
            song2= 'resource/beep0.5s_freq400.wav'
            song3='resource/beep0.5s_freq1000.wav'
    ...
    print( failnumss)
    print( successnumss)
    dataload(scores,hards,times/60,trialnumss[0:-1],failnumss,successnumss) #帧数和时间要进行换算
...
#     display_surface = pygame.display.set_mode((pygame.display.Info().current_w, pygame.display.Info().current_h), FULLSCREEN)
