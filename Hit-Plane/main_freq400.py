...
#显示时间会清零，所以放在读数据函数之后
def showtime(frame_clock,score,level,life_num):
    print("时间为%i,分数为%i,游戏级别为%i,剩余生命数量%i" %(frame_clock,score,level,life_num))
    frame_clock=0
...
#显示时间会清零，所以放在读数据函数之后
def showlevel(score,level):
    print("分数为%i,游戏级别为%i" %(score,level))
    frame_clock=0
...
def counthard(level,score,life_num):
    hard=level 
    print("游戏难度系数为：%i" %hard)


# In[7]:

trialnum=0 #每个被试做单个游戏的次数
successnum=0 #成功trial数目
failnum=0#失败trial数目，减一为正确数目
hard=0 #初始任务难度
times=[] #每个trial所需时间
hards=[] #每个trial难度，初始为0
scores=[]
trialnums=[]
successnums=[]
failnums=[]
...
failn=[]
successn=[]
successnum_limit=15   #设置成功次数限制，到15则退出游戏
#初始游戏结束时间和成功完成trial的分数要求
frame_limit=2400 #2400
score_limit=40000 #40000  25000
score_interval=2000
#统计分数************************
...
# 进行时间检测，满一分钟则退出游戏
def timecontrol(frame_clock,hard,life_num,frame_limit,successnum_limit):
    global successnum
    global failnum
    global trialnum
    global score,level,score_limit
    if frame_clock>=frame_limit and score>=score_limit and successnum<successnum_limit:
        print("时间已满%i，您已成功完成任务，继续下一次任务" %frame_limit)
        showtime(frame_clock,score,level,life_num)
        successnum+=1 #成功次数加1
        trialnum+=1
        print("Successnum=%i  "%successnum)
        showtime(frame_clock,score,level,life_num)
        main()
    elif frame_clock>=frame_limit and score <score_limit and successnum<successnum_limit:
        print("时间已满%i，您未能成功完成任务，重新开始" %frame_limit)
        showtime(frame_clock,score,level,life_num)
        failnum+=1 #失败次数加1
        trialnum+=1
        print("FailNum=%i"%failnum)
        showtime(frame_clock,score,level,life_num)
        main()
    elif frame_clock>=frame_limit and successnum>=successnum_limit:
        print("时间已满%i，您已成功完成任务，退出游戏" %frame_limit)
        print("trialnum=%i"%trialnum)
        showtime(frame_clock,score,level,life_num)
        pygame.quit()
        sys.exit()
...
def load_music():
    song2 = pygame.mixer.Sound("sound\\beep0.5s_freq400.wav")
    song3 = pygame.mixer.Sound("sound\\beep0.5s_freq1000.wav")
    pygame.mixer.init()
    soundway=pygame.mixer.Sound(song2)
    #soundway=pygame.mixer.Sound(song3)
    soundway.play()
...
def rungame():
    global successnum,successnums,successn,failnum,failnums,failn,trialnum,trialnums,trialn
    global score,scores,hard,hards,times
    global score_limit,score_interval
    #初始关卡和分数设计
    level=1
    score = 0
...
#     level = startlevel
    #--------循环主体---------------
    while running:
#         print("Running %i" %frame_clock)
#       开始进行时间检测，时间满则退出游戏
#       timecontrol(frame_clock,hard,life_num,frame_limit,successnum_limit)
        if frame_clock>=frame_limit and score>=score_limit and successnum<successnum_limit:
            print("时间已满%i，您已成功完成任务，继续下一次任务" %frame_limit)
            load_music()
            showtime(frame_clock,score,level,life_num)
            ...
            showdata(scores,hards,times/60,trialnum,failnum,successnum)
            #加载数据
            dataload(scores,hards,times/60,trialn,failn,successn)  
            #********************************************
            score=0
#             score_interval=score_interval+1000
            score_limit += score_interval  #需要增加10000分才算成功
            hard+=1
            print("Successnum=%i  hard=%i score_limit=%i,score=%i,score_interval=%i"%(successnum,hard,score_limit,score,score_interval))
            print("   ")
            main()
        elif frame_clock>=frame_limit and score <score_limit and successnum<successnum_limit:
            #直接进入结束界面
            ...
            print("时间已满%i，您已成功完成任务，退出游戏" %frame_limit)
#             trialnum+=1
            print("trialnum=%i  hard=%i  score_limit=%i ,score=%i,score_interval=%i"%(trialnum,hard,score_limit,score,score_interval))
            showtime(frame_clock,score,level,life_num)
            pygame.quit()
            sys.exit()
...
#         #增加游戏难度
#         makegamehard(level,score,small_enemies,mid_enemies,big_enemies,enemies)
            #根据得分更改游戏等级
        #     showlevel(score,level)
        ...
        if life_num and not paused:
            ...
            for each in big_enemies:
                if each.active:
                    ...
                    #绘制血槽
                    pygame.draw.line(screen,BLACK,                        (each.rect.left,each.rect.top - 5),                        (each.rect.left,each.rect.top - 5),2)
                    #当生命对于20%显示绿色，否则显示红色
        ...
        #绘制游戏结束画面
            ...
            if pygame.mouse.get_pressed()[0]:
                ...
                if again_rect.left < pos[0] < again_rect.right and again_rect.top < pos[1] <again_rect.bottom:
                    ...
                    score=0
        #             score_interval=score_interval-1000
                    score_limit-=score_interval #需要减少2000分才算成功
                    hard-=1
                #分数清零
        #             elif level<2:
        #                 score=level*10000+1-10000
                    print("FailNum=%i,hard=%i,score_limit=%i,score=%i,score_interval=%i"%(failnum,hard,score_limit,score,score_interval))
                    print("   ")
                    main()
                #如果用户点击“结束游戏”
